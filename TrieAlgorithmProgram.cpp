...
typedef struct Dictionary{
	struct Dictionary* children[SIZE];
	bool isEnd;
} Dictionary;
...
void push(Dictionary* root, char* slang){
	Dictionary* curr = root;
	for(int i = 0; slang[i] != '\0'; i++){
		int idx = slang[i] - 'a';
		if(!curr->children[idx]){
			curr->children[idx] = createDict();
		}
...
void searchByPrefix(Dictionary* root) {
    char prefix[100];
